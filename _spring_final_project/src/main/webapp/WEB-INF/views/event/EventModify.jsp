...
<jsp:include page="/WEB-INF/views/common/header.jsp"></jsp:include>
	...
	<div class="container">
		...
		<div id="tab-2" class="tab-content">
			...
			<div class="attendance-setting-div" style="display: none;">
				...
				<table class="attendance-table">
					<tr>
						<th>총 출석체크 일 수</th>
						<td><input type="text" name="atvo.maxAttendanceCount" value="0" id="atvo.maxAttendanceCount">
						<span class="td-span">일</span>
						</td>
					</tr>
					<tr>
						<th>출석체크 1회당 지급 포인트</th>
						<td><input type="text" name="atvo.point" value="0" id="atvo.point">
						<span class="td-span">포인트</span>
						</td>
					</tr>
					<tr>
						<th>특정 횟수마다 출석시 특별보상 지급</th>
						<td><input type="text" name="atvo.specialPointCount" value="0" id="atvo.specialPointCount">
						<span class="td-span">회</span>
						</td>
					</tr>
					<tr>
						<th>특별 보상</th>
						<td><input type="text" name="atvo.specialPoint" value="0" id="atvo.specialPoint">
						<span class="td-span">포인트</span>
						</td>
					</tr>
					<tr>
						<th>개근상</th>
						<td><input type="text" name="atvo.fullAttendancePrize" value="0" id="atvo.fullAttendancePrize">
						<span class="td-span">포인트</span>
						</td>
					</tr>
				</table>
		...
		<div id="tab-3" class="tab-content">
			<div>
				<label>1. 제목</label>
				<input type="text" name="evo.evName" id="evName" value="${evo.evName}">
			</div>
			<div>
				<label>2. 기간</label>
				<input type="datetime-local" name="evo.evStart" id="evStart" value="${evo.evStart }"> ~
				<input type="datetime-local" name="evo.evEnd" id="evEnd" value="${evo.evEnd }">
			</div>
			<div style="display: none;">
				<select name="evo.evType" id="evSelec">
						<option>선택...</option>
						<option value="rouletteEv" ${evo.evType eq "rouletteEv" ? 'selected':''}>룰렛</option>
						<option value="AttendanceEv" ${evo.evType eq "AttendanceEv" ? 'selected':''}>출석체크</option>
				</select>
			</div>
			<div>
				<label>3. 내용</label>
				<textarea id="summernote" name="evo.evContent">${evo.evContent}</textarea>
			</div>
			<input type="hidden" value="${evo.evNo}" name="evo.evNo">
			<div class="tab3-low">
				<button type="button" class="prevButton">이전</button>
